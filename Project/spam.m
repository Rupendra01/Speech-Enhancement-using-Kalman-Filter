function varargout = pr_rou_gui(varargin)
...
% Last Modified by GUIDE v2.5 30-Nov-2021 18:23:42
...
plot(handles.axes5,Time, Output)
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global Output Fs o_player
im=imread('project_image.jpg');
axes(handles.axes6);
imshow(im)
o_player = audioplayer(Output, Fs);
play(o_player);
%sound(Output,Fs);
...
% [filename pathname] = uigetfile({'*.wav'}, 'Select File');
% fullfilename = strcat (pathname, filename);
[Input, Fs] = audioread('handle1.wav');
%[Input,Fs] = audioread(filename);
im=imread('project_image.jpg');
axes(handles.axes1);
imshow(im);
